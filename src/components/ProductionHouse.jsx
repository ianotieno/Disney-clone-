      ...
      {productionHouseList.map((item) => (
        ...
        >
          <video
            ...
            playsInline
            muted
            className="absolute z-0 top-0 left-0 w-full h-full rounded-md opacity-0 hover:opacity-50 transition-opacity duration-300"
          />
          <img
            src={item.image}
            alt={`Production house ${item.id}`}
            className="w-full z-[1] rounded-md opacity-100"
          />
        </div>
      ))}
    </div>
